...
TEST(CFXDIBBaseTest, RustAlphaTransformMatchesCppReference) {
  auto source = CreateConversionBitmap(FXDIB_Format::k8bppMask, false);
  ASSERT_TRUE(source);
  ...
  for (const auto& matrix : kMatrices) {
    ...
    {
      fxge::ScopedRustDibImplementationForTesting implementation(false);
      reference = source->TransformTo(matrix, &reference_left, &reference_top);
    }
