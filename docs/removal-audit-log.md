...
| removed | Alternative render and font backends | renderer/font-backend selection and the Skia premultiplied bitmap format are absent | Skia, the experimental Fontations/Skrifa bridge, Rust-PNG adapter, Skia checkout dependency, and Skia-specific golden images are removed; AGG + FreeType remain | The static product has one supported raster and font stack while Rust replacements are introduced module by module behind the C API. | Light header compile and source/build graph scans pass with no active Skia or Skrifa dependency. |
| removed | Residual XFA codecs and CSS | none | `core/fxcrt/css/`, XFA BMP/GIF/PNG/TIFF codecs, progressive image decoder, XFA TrueType table parser, and data-availability parser helpers are removed | These paths were only reachable through the rejected XFA or progressive-loading configurations. | `pdf_enable_xfa=true` remains rejected; retained build files have no XFA codec or CSS target. |
| removed | Progressive loading and rendering | `fpdf_dataavail.h`, `fpdf_progressive.h`, `FPDFAvail_*`, and `FPDF_RenderPageBitmap_*` pause/continue calls are absent | availability simulation, download hints, pause adapter, progressive renderer, and their tests/fuzzer are removed; rendering loads complete documents and renders synchronously | Static server use does not need incremental download or pause/resume state machines. | Light header compile passes; retained render tests use synchronous `FPDF_RenderPageBitmap()`. |
...
| removed | macOS x64/Intel support | n/a | public docs no longer advertise macOS x64/Intel support; macOS x86-only expectation fallback, suppression entries, and golden PNGs removed | macOS Intel is not a target platform. | `docs/platform-support.md` lists macOS x64/Intel as out of scope; README lists macOS arm64 as the supported macOS target; static validation rejects stale macOS x86 expectation assets. |
