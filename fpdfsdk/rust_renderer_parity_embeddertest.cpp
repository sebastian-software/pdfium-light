...
TEST_P(RustRendererParityEmbedderTest, CandidateMatchesCppReferenceExactly) {
  ...
  if (test_case.name == "DashedLines") {
    ...
    EXPECT_TRUE(
        fxge::AggTraceHasStrokeMatrixPlansForTesting(reference_agg_trace));
  }
  ...
  if (test_case.name == "HelloWorldNoNativeText") {
    ...
    EXPECT_TRUE(
        fxge::GlyphTraceHasBitmapLookupPlansForTesting(reference_glyph_trace));
    EXPECT_TRUE(
        fxge::GlyphTraceHasFreeTypeLoadPlansForTesting(reference_glyph_trace));
  }
}
